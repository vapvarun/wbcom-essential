/**
 * EDD Checkout Recommendations Block - Editor Component
 *
...
export default function Edit( { attributes, setAttributes, clientId } ) {
	const { uniqueId, recommendationCount } = attributes;
	...
	const blockProps = useBlockProps( {
		className: `wbe-block-${ uniqueId } wbcom-essential-edd-checkout-recommendations-editor`,
	} );

	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Recommendation Options', 'wbcom-essential' ) } initialOpen={ true }>
					<RangeControl
						label={ __( 'Number of Recommendations', 'wbcom-essential' ) }
						value={ recommendationCount }
						onChange={ ( value ) => setAttributes( { recommendationCount: value } ) }
						min={ 1 }
						max={ 6 }
					/>
				</PanelBody>
			...
			<div { ...blockProps }>
				<div style={ { border: '1px dashed #cbd5e1', borderRadius: 8, padding: '20px 16px', background: '#f8fafc' } }>
					<p style={ { margin: 0, fontSize: 13, fontWeight: 600, color: '#475569' } }>
						{ __( 'EDD Checkout Recommendations', 'wbcom-essential' ) }
					</p>
					<p style={ { margin: '4px 0 0', fontSize: 12, color: '#94a3b8' } }>
						{ __( 'Cart-based product suggestions render here on the frontend.', 'wbcom-essential' ) }
					</p>
				</div>
			</div>
		</>
