...
( function () {
	...
	var container, state, debounceTimer;

	function init() {
		container = document.getElementById( 'wbcom-product-catalog' );
		if ( ! container ) {
			return;
		}
	...
	function render() {
		...
		if ( state.showCategory ) {
			...
			addOption( catSelect, '0', 'All Categories', state.category === 0 );
		...
		if ( state.loading ) {
			for ( var i = 0; i < state.perPage; i++ ) {
				...
				skel.appendChild( el( 'div', 'wbcom-catalog__card-image' ) );
				skel.appendChild( el( 'div', 'wbcom-catalog__card-title' ) );
				skel.appendChild( el( 'div', 'wbcom-catalog__card-excerpt' ) );
				skel.appendChild( el( 'div', 'wbcom-catalog__card-price' ) );
	...
	function buildCard( product ) {
		...
		// Price — server-rendered currency string (admin content, trusted).
		var priceDiv = el(
			'div',
			product.is_free
		...
		btn.textContent = product.is_free ? 'Download Free' : 'View Product';
	...
	function updateUrl() {
		var params = [];
		if ( state.category ) {
			params.push( 'cat=' + state.category );
		}
		if ( state.search ) {
			params.push( 'q=' + encodeURIComponent( state.search ) );
		}
		if ( state.priceRange !== 'all' ) {
			params.push( 'price=' + state.priceRange );
		}
		var sortVal = getCurrentSortValue();
		if ( sortVal !== 'title' ) {
			params.push( 'sort=' + sortVal );
		}
	...
	// Helper: create a div with className.
