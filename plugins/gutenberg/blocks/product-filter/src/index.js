...
registerBlockType( 'wbcom-essential/product-filter', {
	edit( { attributes, setAttributes } ) {
		const { filters, sticky, stopAtCover } = attributes;
		const blockProps = useBlockProps( {
			className: 'wbcom-product-filter-editor',
		} );
				...
				<InspectorControls>
					...
					>
						{ filters.map( ( filter, i ) => (
							<div
								key={ i }
								className="wbcom-filter-item-panel"
							>
	...
	save( { attributes } ) {
		...
		return (
			<div
				...
				data-stop-at-cover={ stopAtCover ? 'true' : 'false' }
			>
				<div className="wbcom-filter-wrap">
					{ filters.map( ( filter, i ) => (
						<button
