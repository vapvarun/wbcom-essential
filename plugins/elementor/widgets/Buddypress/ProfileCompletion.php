...
class ProfileCompletion extends \Elementor\Widget_Base {
	...
	public function profile_calculate_profile_percentage( $user_id, $settings ) {
		 ...
		 */
		$is_profile_photo_disabled = bp_disable_avatar_uploads();
		if ( ! $is_profile_photo_disabled && isset($settings['profile_photo']) && $settings['profile_photo'] == 'yes' ) {
		...
		$is_cover_photo_disabled = bp_disable_cover_image_uploads();
		if ( ! $is_cover_photo_disabled && isset($settings['cover_photo']) && $settings['cover_photo'] == 'yes' ) {
		...
		if ( !empty($profile_groups)) {
			...
			foreach ( $profile_groups as $single_group_details ) {
				 ...
				 */
				if ( isset($settings[ 'profile_field_' . $group_id ]) && $settings[ 'profile_field_' . $group_id ] != 'yes' ) {
					continue;
				}
	...
	function get_user_profile_progress_formatted( $user_progress_arr, $settings ) {
