...
| Date | Description |
|------|-------------|
| 2026-06-20 | EDD account findability: added a search + filter + sort + result-count toolbar to the Downloads and License Keys tabs (power customers own 100+ items). Downloads: name search, Available/Expired filter, Recently-purchased/Name sort (PHP-side). Licenses: name search (title→download_id), Active/Expired filter, Recently-added/Expiry/Status sort - all DB-level so `count()` + pagination stay correct at scale. Instant AJAX (results-region swap keeps search focus); degrades to full reload without JS. Downloads perf restructure: build a light index → filter/sort/slice → resolve signed file URLs for the visible 12 ONLY (was signing URLs + license-checking all 100+ per render). New `data-edd-*` attrs whitelisted in `wbcom_essential_kses_form()`. Verified at 137 orders / 135 licenses. Scale seeder: `scripts/seed-edd-scale.php`. |
| 2026-06-09 | Fix: EDD account Downloads tab served the "Expired License" HTML error page (saved as a file via the `download` attr) instead of a ZIP when a licensed product/bundle's license was expired. Now gates file links through EDD SL `license_can_download()` and shows a "License expired - Renew" state. Access follows LICENSE validity, not subscription status, so a cancelled-but-still-valid subscription keeps downloading. Applies to bundle children + regular items; empty-URL anchors guarded. Same `license_can_download()` guard added to the claim-free REST endpoint (free plugins can carry a lifetime license). |
